...
// Simulador da LP /parceria-ufms — reaproveita o LAYOUT do simulador do
// site institucional (components/site/simulator.tsx: card com flip 3D,
// mesmos estilos/inputs), mas a LÓGICA é outra e não compartilha nada com
// ele: sem CRM (sem criar negócio, sem agendamento, sem notificação
// interna) e sem a etapa de "horários disponíveis". São só duas telas —
// (1) tipo/valor do consórcio e (2) dados pessoais — e ao confirmar a
// segunda, o payload vai direto pro webhook dedicado da parceria
// (/api/simulador-webhook-ufms), ponto final.
...
export interface SimuladorPayloadUFMS {
  ...
  // Rótulo de origem do lead — só entra no corpo do webhook, não tem CRM
  // pra identificar por trás dele nesta versão.
  origem: string
}

// Desvia o CTA final pro WhatsApp do consultor dedicado da parceria — sem
// isso o componente não sabe pra qual número mandar (não existe link de
// WhatsApp geral vindo do backend aqui, ao contrário do simulador do site
// institucional).
interface WhatsappOverride {
  telefone: string
  montarMensagem: (payload: SimuladorPayloadUFMS) => string
}

interface SimulatorUFMSProps {
  origemLabel?: string
...
export function SimulatorUFMS({
  origemLabel = 'Simulador Online - Parceria UFMS',
  whatsappOverride,
  ...
  // Um único flip: frente (tipo/valor) ⇄ verso (dados pessoais). Sem flip
  // interno de agendamento — essa etapa não existe mais aqui.
  ...
  const [lastPayload, setLastPayload] = useState<SimuladorPayloadUFMS | null>(null)
  ...
  const construirPayload = (): SimuladorPayloadUFMS => ({
  ...
  // Único gatilho real da simulação: valida os dados pessoais e manda
  // direto pro webhook — nada de negócio no CRM, nota, notificação interna
  // ou reserva de horário no meio do caminho.
  const enviarSimulacao = async () => {
    ...
    try {
      ...
      fpixel.event('Lead', {
        content_name: 'Simulador Concluído - Parceria UFMS',
  ...
  return (
    <div className="perspective-container">
      ...
      >
            ...
            <button
              ...
              onClick={() => {
                ...
                fpixel.event('InitiateCheckout', {
                  content_name: 'Simulador Iniciado - Parceria UFMS',
        ...
        {/* LADO B: Dados Pessoais + Estados de Feedback — sem etapa de
            agendamento. O botão final já dispara enviarSimulacao direto. */}
        <div
          ref={backRef}
        ...
        >
          ...
          {submitStatus === 'idle' || submitStatus === 'loading' ? (
            <div className="flex flex-col justify-between gap-6">
              ...
              {/* Botão de Envio para Webhook — gatilho real e único da
                  simulação, direto daqui (sem etapa de agendamento). */}
                ...
                <>
                  ...
                  {linkWhatsappGerado && (
                    <a
                      ...
                      onClick={() => fpixel.event('Contact', { content_name: 'Pós-Simulação WhatsApp - UFMS' })}
